...
export class InstitutionSettingsWhiteLabelPage extends BasePage {
  constructor(page) {
    ...
    // Main Navigation (used in gotoInstitutionSettingsWhiteLabel)
    this.institutionSettingsTab = page
      .locator('a')
      .filter({ hasText: 'Institution Settings' });

    // Settings Navigation (used in gotoInstitutionSettingsWhiteLabel)
    ...
    this.uploadPhotoDescriptionFavicon = page.getByText(
      'We suggest using a 32x32'
    );

    // Cover image
    this.coverImageHeader = page.getByText('Cover image').first();
    this.coverImageAvatar = page
      .locator('div')
      .filter({ hasText: /^Cover image preview$/ });
    this.uploadPhotoButtonCoverImage = page.getByRole('link', {
      name: 'Upload Upload cover image',
    });
    this.resetPhotoButtonCoverImage = page
      .getByRole('link', { name: 'DeferredPayment Reset to' })
      .first();
    this.uploadPhotoDescriptionCoverImage = page.getByText(
      'Upload a new cover image to'
    );
    ...
    this.resetToDefaultBrandingButton = page
      .getByRole('link', { name: 'Reset to default' })
      .nth(1);
    this.brandingCard = page.getByTestId('card').nth(1);

    this.primaryColorButton = page.locator(
      'input[name="whitelabel\\.primaryColor"]'
    );
    ...
    this.modalCancelButton = page
      ...
      .first();
    this.modalSelectButton = page
      .getByRole('button', { name: 'Select' })
      .first();

    this.mainButtonTextColorButton = page.locator(
      'input[name="whitelabel\\.buttonTextColor"]'
    );
    ...
    this.scheduleAppointmentText = page
      .getByRole('paragraph')
      .filter({ hasText: 'Schedule an appointment' });
    this.scheduleAppointmentCard = page.getByTestId('card').nth(2);
    this.seeProviderNowText = page
      .getByRole('paragraph')
      .filter({ hasText: 'See a provider now' });
    ...
    this.invalidImageErrorMessage = page
      .getByText('Please upload a valid image file (JPEG, PNG, SVG).')
      .first();
    this.organizationNameRequiredError = page.getByText(
    ...
    this.alphanumericValidationError = page.getByText(
      'Text fields can include alphanumeric characters'
    );
    this.subdomainValidationError = page.getByText(
      'A subdomain can only contain letters, numbers, or hyphens'
    );
    this.fileSizeExceedsError = page
      .getByText('File size exceeds the maximum limit of 5MB')
      .first();
    ...
    this.scheduleAppointmentColorPickers =
      this.scheduleAppointmentCard.getByTestId('color-picker');
    this.seeProviderNowColorPickers =
      this.seeProviderNowCard.getByTestId('color-picker');
    this.resetToDefaultPatientBooking = page
      .getByRole('link', { name: 'DeferredPayment Reset to' })
      .nth(2);
    this.resetToDefaultSeeProviderNow = page
      .getByRole('link', { name: 'DeferredPayment Reset to' })
      .nth(3);
    this.tooltipCancelButton = page
      .getByRole('tooltip')
      .getByRole('button', { name: 'Cancel' });
  ...
  async gotoInstitutionSettingsWhiteLabel() {
