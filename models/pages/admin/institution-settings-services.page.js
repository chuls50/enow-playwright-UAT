...
export class InstitutionSettingsServicesPage extends BasePage {
  ...
  async resetState() {
    ...
    if (await this.page.locator('div').filter({ hasText: /^45 minutes$/ }).isVisible()) {
        await this.page.locator('div').filter({ hasText: /^45 minutes$/ }).getByRole('button').click();
    }
    
    // If 60 minutes duration is visible, click the X to remove it
    if (await this.page.locator('div').filter({ hasText: /^60 minutes$/ }).isVisible()) {
        await this.page.locator('div').filter({ hasText: /^60 minutes$/ }).getByRole('button').click();
