...
export class MyAccountPage extends BasePage {
    constructor(page) {
        ...
        this.deletePhotoButton = page.getByRole('button', { name: 'Trash Delete photo' });        //Profile Details Section
        ...
        this.profileDetailsEditButton = page.locator('div').filter({ hasText: /^Profile detailsEdit$/ }).getByRole('button');
        
        // License to practice section
        this.licenseToPracticeSection = page.getByText('License to practice');
        this.licenseToPracticeButton = page.locator('div').filter({ hasText: /^License to practiceEdit$/ }).getByRole('button');
        ...
        this.editProfileMedicalSpecialtyDropdownButton = page.locator('div').filter({ hasText: /^Allergologist$/ }).getByRole('button');
        this.editProfileMedicalSpecialtyDropdownMenu = page.locator('div').filter({ hasText: /^Medical specialtiesAngiologist$/ }).getByRole('button');
        ...
        this.editLicenseModalLicense1CountryDropdown = page.locator('div').filter({ hasText: /^CountryAfghanistan$/ }).getByTestId('custom-select-item-wrapper');
        ...
        this.editLicenseModalLicense1StateDropdown = page.locator('div').filter({ hasText: /^StateSelect state$/ }).getByTestId('custom-select-item-wrapper');
    ...
    async gotoProviderMyAccount() {
        await this.page.goto(`${process.env.PROD_URL}/account-settings/my-account`);
    ...
    async openEditLicenseToPracticeModal() {
        try {
            ...
            if (await this.licenseToPracticeButton.isVisible()) {
                ...
                const fallbackButton = this.page.locator('div').filter({ hasText: /^License to practice$/ }).getByRole('button');
            ...
            const removeLicense3Button = this.page.getByRole('link', { name: 'Remove License 3' });
            if (await removeLicense3Button.isVisible()) {
                ...
                await this.licenseToPracticeButton.waitFor({ state: 'visible', timeout: 5000 });
                await this.licenseToPracticeButton.click();
            }
        } catch (error) {
    ...
    async openEditProfileSlideOut() {
        try {
            ...
            // If primary button isn't visible, try the fallback approach
            const fallbackButton = this.page.locator('div').filter({ hasText: /^Profile details$/ }).getByRole('button');
            await fallbackButton.waitFor({ state: 'visible', timeout: 5000 });
            await fallbackButton.click();
            }
        } catch (error) {
    ...
    async selectMedicalSpecialtyInEditProfile(specialty) {
        const medicalSpecialtyDropdown = this.page.getByRole('textbox', { name: 'Select medical specialities' });
    ...
    async selectLanguageInEditProfile(language) {
        const languagesSpokenDropdown = this.page.getByRole('textbox', { name: 'Languages spoken' });
