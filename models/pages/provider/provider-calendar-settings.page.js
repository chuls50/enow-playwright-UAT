...
export class CalendarSettingsPage extends BasePage {
    constructor(page) {
        ...
        this.availabilityAddIconMonday = page.locator('div').filter({ hasText: /^Monday:—:$/ }).getByRole('link').nth(1);
        this.availabilityDeleteMondayChrome = page.locator('div').filter({ hasText: /^:—::—:$/ }).getByRole('link').nth(1);
        this.availabilityDeleteMondayMobile = page.locator('div').filter({ hasText: /^12:00 AM—11:59 PMhh:mm a—hh:mm a$/ }).getByRole('link').nth(1);
    ...
    async gotoCalendarSettingsPage() {
        await this.page.goto(`${process.env.PROD_URL}/account-settings/calendar`);

        // Wait for spinner to disappear if present
        await this.waitForSpinnerToDisappear();

        // Wait for Calendar settings page to load
